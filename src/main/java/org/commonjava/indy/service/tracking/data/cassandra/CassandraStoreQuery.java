...
import static org.commonjava.indy.service.tracking.data.cassandra.CassandraStoreUtil.TABLE_AFFECTED_STORE;
import static org.commonjava.indy.service.tracking.data.cassandra.CassandraStoreUtil.TABLE_STORE;
    ...
    public void init()
    {
        ...
        preparedSingleArtifactStoreQuery = session.prepare(
                        "SELECT packagetype, storeType, namehashprefix, name, description, transientMetadata, metadata, disabled, disableTimeout, pathStyle, pathMaskPatterns, authoritativeIndex, createTime, rescanInProgress, extras FROM "
                                        ...
                                        + " WHERE typekey=? AND namehashprefix=? AND name=?" );
