...
public class StoreDataSetupAction
{
    ...
    public void start()
    {
        ...
        if ( !trackingConfig.queryCacheEnabled() )
        {
            logger.info( "The query cache is not enabled, all query result will be directly retrieved from underlying data store." );
        }
        else
        {
            logger.info( "The query cache is enabled, some query results will be cached for performance consideration." );
        }
        ...
        try
        {
            ...
            {
                final RemoteRepository central = new RemoteRepository( MAVEN_PKG_KEY, "central",
