...
import static org.commonjava.indy.service.tracking.data.metrics.NameUtils.getSupername;
import static org.commonjava.indy.service.tracking.data.metrics.TraceManager.INDY_METRIC_ISPN;
...
public class CacheProducer
{
    ...
    private EmbeddedCacheManager mergedCachesFromConfig( EmbeddedCacheManager cacheMgr, String config, String path )
    {
        ...
        if ( mgr == null )
        {
            ...
            {
                throw new RuntimeException(
                                String.format( "Failed to construct ISPN cacheManger due to %s xml stream read error.",
                                               path ), e );
            }
        }
